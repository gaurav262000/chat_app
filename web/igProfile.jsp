...
.ImgTab1{
  ...
  <div class="file-field">
   ...
   <div class="profileTop">
     <div class="profileImg">
         <img src="getImage.jsp?name=<%=user.getName() %>"/>
     </div>
  ...
  String from_user_name=request.getSession().getAttribute("Name").toString();            
          PreparedStatement ps=DBUtils.getPreparedStatement("select count(*) from feedImage where from_user_name='"+from_user_name+"'");
...
<% } 
 PreparedStatement ps4=DBUtils.getPreparedStatement("select count(*) from friendList where to_user_name='"+from_user_name+"'");
...
<td><%=rs4.getInt("count(*)") %></td>
           ...
           PreparedStatement ps5=DBUtils.getPreparedStatement("select count(*) from friendList where from_user_name='"+from_user_name+"'");
                ResultSet rs5=ps5.executeQuery();

                while(rs5.next()){ %>
           <td><%=rs5.getInt("count(*)") %></td>
<%
}
           %>
         </tr>
  ...
  <div class="profileDetail">
    <p><%=user.getName() %></p>
    <p><%=user.getBio() %><br>
      <%=user.getDOBDay()%>/<%=user.getDOBMonth()%>/<%=user.getDOBYear()%><br>
   contact:- <%=user.getContact() %><br>
      <br>
            ...
            <div class="story" style="text-align: center; cursor: pointer; ">
    Add new post 
  </div>
  <div class="tabBox">
  ...
  <div class="ImgTab" style="display: none;">
        <%
           ...
           PreparedStatement ps2=DBUtils.getPreparedStatement("select * from feedImage where from_user_name='"+from_user_name+"' ORDER BY feed_id desc ");
  ...
  String from_user_name1=request.getSession().getAttribute("Name").toString();
           ...
           PreparedStatement ps1=DBUtils.getPreparedStatement("select * from feedImage where from_user_name='"+from_user_name1+"' ORDER BY feed_id desc");
  int k=0;   
 ...
 <div style="padding: 10px;">
       <img src="getImage.jsp?name=<%=user.getName() %>" class="rounded-circle friend-pic" style="height: 35px;width: 35px;"/>
 </div>
 <span style="padding: 10px;
margin-top: 2px;">
     <a href="#" style="text-decoration: none;
color: black;">  <%=user.getName() %></a>
 </span>
 <div style="padding: 10px;
margin-left: 65%;
margin-top: 5px;"><i class="fas fa-ellipsis-v"></i></div>
  ...
  <div style="    padding: 2px 2px 1px;">
   ...
   <i id="like<%=k%>" class="far fa-heart dislike<%=k%>" style="color: black;font-size: 31px; cursor: pointer;" onclick="like(this.id,'<%=rs1.getInt(1) %>','<%=rs1.getString(2)%>')"></i>    
