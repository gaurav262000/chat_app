 ...
 * @author Lenovo
 */
@WebServlet(urlPatterns = {"/imagestorer"})
@MultipartConfig(maxFileSize = 16177215)
public class imagestorer extends HttpServlet {
      ...
      //  HttpSession session=request.getSession();  
		...
		Part part =request.getPart("image");
         System.out.println(part +"4byy im in msgcollector");
                String to_user=request.getParameter("touserName") ; 
     ...
     //   String originalString=request.getParameter("date") ;
		...
		if(part != null){
			try{
			    ...
			    PreparedStatement ps = con.prepareStatement("insert into msgcollector(msgImage,to_user, from_user_name,date,time) values(?,'"+to_user+"', '"+from_user_name+"','"+originalString.format(d)+"','"+time.format(d)+"') ");
		...
		System.out.println("2nd test");
                ...
                response.sendRedirect("chat2.jsp?to_user="+to_user+"&id="+id+"");
                }
